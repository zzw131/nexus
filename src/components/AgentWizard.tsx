...
export default function AgentWizard({ onClose, onComplete }: AgentWizardProps) {
  ...
  const modelsByRuntime: Record<
    string,
    { id: string; name: string; window: string; speed: string }[]
  > = {
    llama: [
      {
        id: "qwen-14b",
  ...
  const currentModels = config.runtime
    ? modelsByRuntime[config.runtime] || []
    : [];
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-zinc-200/50 dark:border-zinc-800/50 flex flex-col gap-3">
          ...
          <div className="flex items-center gap-2">
            {[1, 2, 3, 4, 5].map((i) => (
              <div
                key={i}
                className="h-1 flex-1 rounded-full bg-zinc-100 dark:bg-zinc-800 overflow-hidden"
              >
                <motion.div
                  initial={{ width: 0 }}
                ...
                <div className="grid grid-cols-2 gap-3">
                  {runtimes.map((rt) => (
                    <div
                      key={rt.id}
                      onClick={() =>
                        setConfig({ ...config, runtime: rt.id, model: "" })
                      }
            ...
            {step === 4 && (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-zinc-500 mb-4">
                    根据您的引擎 (
                    {runtimes.find((r) => r.id === config.runtime)?.name})
                    选择适配模型
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="mt-3">
                    ...
                    <input
                      type="text"
                      value={
                        currentModels.some((m) => m.id === config.model)
                          ? ""
                          : config.model
                      }
                      onChange={(e) =>
              ...
              >
                ...
                <div className="p-4 rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 space-y-3 relative overflow-hidden">
                  ...
                  <div className="grid grid-cols-2 gap-y-2 mt-2 pt-2 border-t border-zinc-200 dark:border-zinc-800 font-mono text-[10px]">
                    ...
                    <div className="font-bold text-zinc-800 dark:text-zinc-200 text-right truncate overflow-hidden">
                      {config.model}
                    </div>
                  </div>
                </div>
                ...
                <div className="p-3 bg-rose-50/50 dark:bg-rose-950/20 border border-rose-200/50 dark:border-rose-900/30 rounded-lg text-rose-600 dark:text-rose-400 text-xs font-semibold flex gap-2 leading-relaxed">
                  ...
                  ⚠️ Agent
                  创建后，引擎类型和运行位置不可修改。确认后将立即在本地注入节点配置。
                </div>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="px-6 py-4 border-t border-zinc-200/50 dark:border-zinc-800/50 flex items-center justify-between bg-zinc-50/50 dark:bg-zinc-900/30">
          ...
          <button
            onClick={handleNext}
            disabled={
              (step === 1 && (!config.name || !config.alias)) ||
              (step === 3 && !config.runtime) ||
              (step === 4 && !config.model)
            }
