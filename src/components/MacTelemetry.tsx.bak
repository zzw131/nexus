...
export function HardwareTelemetryCard() {
  const [status, setStatus] = useState<"loading" | "error" | "normal">(
    "loading",
  );
    ...
    <div className="p-3 bg-zinc-50 rounded-xl border border-zinc-200">
      <div className="flex items-center justify-between mb-1">
        <span className="text-[11px] text-zinc-500 dark:text-zinc-400 flex items-center gap-1.5 font-sans font-medium">
          {status === "error" ? (
            <WifiOff className="w-3.5 h-3.5 text-zinc-405 opacity-50" />
          ) : (
            icon
          )}{" "}
          {label}
        ...
        {status === "loading" ? (
          ...
          <span className="text-[11px] font-bold font-mono text-zinc-400 dark:text-zinc-600">
            N/A
          </span>
        ) : (
          <span className="text-[11px] font-bold font-mono text-zinc-800 dark:text-zinc-200">
            {value}
          </span>
        )}
      </div>
    ...
    <div className="p-5 overflow-hidden transition-all duration-300 border bg-white rounded-2xl border-zinc-200 shadow-none flex flex-col justify-between h-full">
      <div>
        <div className="flex items-center justify-between mb-4 gap-2">
          ...
          <div className="flex items-center gap-1 bg-zinc-100 px-2.5 py-1 shrink-0 whitespace-nowrap rounded-full text-[10px] font-mono text-zinc-600 border border-zinc-200 shadow-none">
            {status === "error" ? (
              ...
              <Wifi
                className={`w-2.5 h-2.5 ${status === "loading" ? "text-zinc-400" : "text-emerald-500"}`}
              />
            )}
            <span>
              {status === "error" ? "连接断开" : "Tailscale 隧道连接"}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3.5">
          {renderMetric(
            "CPU 负载率",
            <Cpu className="w-3.5 h-3.5 text-zinc-405" />,
            `${stats.cpuLoad}%`,
            stats.cpuLoad,
            "bg-blue-500",
          )}
          {renderMetric(
            "内存使用率",
            <Database className="w-3.5 h-3.5 text-zinc-405" />,
            `${stats.memoryUsage}%`,
            stats.memoryUsage,
            "bg-[#6366f1]",
          )}
          {renderMetric(
            "核心温度",
            <Thermometer className="w-3.5 h-3.5 text-zinc-405" />,
            `${stats.cpuTemp}°C`,
            (stats.cpuTemp / 100) * 100,
            "bg-[#f43f5e]",
          )}
          {renderMetric(
            "磁盘已用空间",
            <HardDrive className="w-3.5 h-3.5 text-zinc-405" />,
            `${stats.diskUsage}%`,
            stats.diskUsage,
            "bg-[#10b981]",
          )}
...
export function QuickActionsCard() {
  ...
  const triggerAction = async (act: ServerAction) => {
    ...
    if (act.id === "llama_logs" || act.id === "ping") {
      ...
      setTerminalLogs([
        `$ ${act.command}`,
        `已通过隧道 ip 100.83.118.16 成功建立 SSH 网络安全会话连接...`,
      ]);
    }
  ...
  return (
    <div className="flex flex-col md:flex-row items-center justify-between gap-4 py-1.5 bg-transparent text-zinc-650 dark:text-zinc-350">
      ...
      <div className="flex items-center gap-1.5 text-[10px] text-zinc-400 dark:text-zinc-500 select-none flex-wrap md:flex-nowrap">
        <span className="font-mono bg-zinc-200/40 dark:bg-zinc-905/30 px-2 py-0.5 rounded text-zinc-500 dark:text-zinc-400">
          浏览器客户端
        </span>
        ...
        <span className="text-[9px] text-zinc-300 dark:text-zinc-700">↔</span>
        <span className="font-mono bg-blue-50/50 dark:bg-blue-950/20 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded border border-blue-105/10 dark:border-blue-950/10">
          Tailscale 隧道安全组
        </span>
        <span className="text-[9px] text-zinc-300 dark:text-zinc-700">↔</span>
        <span className="font-mono bg-zinc-200/40 dark:bg-zinc-905/30 px-2 py-0.5 rounded text-zinc-500 dark:text-zinc-400">
